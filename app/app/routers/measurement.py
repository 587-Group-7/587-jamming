...
async def create_measurement(measurement: Measurement, db=Depends(database.provide_connection)):
    try:
        await db.execute("INSERT INTO jaminfo (lat, lng, intensity, robot_id) VALUES (:lat, :lng, :intensity, :robot_id)", values={"lat": measurement.lat, "lng": measurement.lng, "intensity": measurement.intensity, "robot_id": measurement.robot_id})
