...
async def create_measurement(measurement: Measurement, db=Depends(database.provide_connection)):
    try:
        await db.execute("INSERT INTO jaminfo (lat, lng, intensity, robotId) VALUES (:lat, :lng, :intensity, :robot_id)", values={"lat": measurement.lat, "lng": measurement.lng, "intensity": measurement.intensity, "robot_id": measurement.robot_id})
...
async def get_measurement_by_id(id: MeasurementId, db=Depends(database.provide_connection)):
    return await db.fetch_one("SELECT lat, lng, intensity, robotId FROM control WHERE id=:id", values={"id": id.id})

@ router.get("/list", status_code=200)
async def list_measurements(request: Request, db=Depends(database.provide_connection)):
    return await db.fetch_all("SELECT lat, lng, intensity, robotId FROM control")
